import type { AlertOptions, AlertVariantItem, Alert } from '../../shared/types/common.ts'
import type { MarkedExtension, Tokens } from 'npm:marked'
import { ucfirst } from '../utils/basicHelpers.ts'
  ...
  return {
    walkTokens(token) {
      ...
      const matchedVariant = resolvedVariants.find(({ type }) =>
        new RegExp(createSyntaxPattern(type), `i`).test((token as Tokens.Blockquote).text),
      )
      ...
      if (matchedVariant) {
        ...
        const firstLine = (token as any).tokens?.[0] as Tokens.Paragraph
        ...
        tokenizer(src: string, _tokens: any) {
          ...
          if (match) {
            ...
            return {
              ...
              raw,
              text: content.trim(),
              tokens: (this as any).lexer.blockTokens(content.trim()),
