      ...
      for (const project of projectsToScrape) {
        try {
          ...
          if (fcResponse.success && fcResponse.data?.markdown) {
            // 获取项目的 stars 数量
            const stars = await this.fetchStarsCount(project.fullName);
            logger.debug(`[GitHub Trending] 项目 ${project.fullName} 有 ${stars} stars`);
          ...
          contents.push(content);
            ...
            logger.info(`[GitHub Trending] ✅ 成功获取详情: ${project.fullName} (⭐ ${stars}, 图片: ${allMedia.length})`);
          }
        } catch (error) {
